...
app.post('/preferences', async (req, res) => {
    ...
    const { userId, preferences } = req.body; // Menerima userId dan array preferences
    ...
    try {
        ...
        // Update kolom preferences. Setter di model akan otomatis stringify array
        await user.update({ preferences: preferences });

        console.log(`Preferences updated successfully for user ID: ${userId}`);
        res.status(200).json({ message: 'Preferences updated successfully', user: { id: user.id, username: user.username, preferences: user.preferences } });
