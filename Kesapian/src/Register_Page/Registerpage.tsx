    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validateForm()) {
            try {
                ...
                const data: { message?: string, user?: { id: number, username: string } } = await response.json();
                if (response.ok) {
                    alert("User registered successfully!");
                    // Simpan userId dan username setelah registrasi
                    if (data.user) {
                        localStorage.setItem('userId', data.user.id.toString());
                        localStorage.setItem('username', data.user.username);
                    }
                    navigate("/preferences"); // Langsung ke halaman preferensi setelah registrasi
                } else {
